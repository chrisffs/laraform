    ...
    <div class="flex flex-row sm:gap-6 min-h-screen">
        ...
        <main class="flex w-full flex-col p-4">
            ...
            <div class="shadow-lg my-6 rounded-[16px] w-max bg-white">
                <div class="py-4 px-6 border-b rounded-t-[16px]">
                    <h1 class="text-xl font-semibold">Add Intern</h1>
                </div>
                <div class="p-6">
                    <form id="addIntern" action="./create_intern" method="post">
                        @csrf
                        <div class="grid grid-cols-3 gap-x-4 gap-y-6 py-2">
                            <div class="flex flex-col">
                                <label for="firstName" class="text-sm text-gray-500 mb-2">First Name</label>
                                <input id="firstName" name="firstName" class="input-md input @error('firstName') input-error @enderror" placeholder="Type here" value="{{old('firstName')}}"/>
                                @error('firstName')
                            ...
                            <div class="flex flex-col">
                                ...
                                <input id="lastName" name="lastName" class="input-md input @error('firstName') input-error @enderror" placeholder="Type here" value="{{old('lastName')}}"/>
                                @error('lastName')
                                <label class="form- label">
                                    <span class="form-label-alt">{{$message}}</span>
                                </label>
                            ...
                            <div class="flex flex-col">
                                ...
                                <input id="midName" name="midName" class="input-md input" placeholder="Type here" value="{{old('midName')}}"/>
                                @error('midName')
                                <label class="form- label">
                                    <span class="form-label-alt">{{$message}}</span>
                                </label>
                            ...
                            <div class="flex flex-col">
                                ...
                                <select id="school" name="school" class="select input-error">
                                    ...
                                    @foreach ($schools as $school)
                                        <option value="{{ $school->schoolName }}" @if(old('school') == $school->schoolName) selected @endif>{{ $school->schoolName }}</option>
                                    @endforeach
                                </select>
                                @error('school')
                                <label class="form- label">
                            ...
                            <div class="flex flex-col">
                                ...
                                <select id="internCategory" name="internCategory" class="select">
                                    ...
                                    @foreach ($programs as $program)
                                        <option value="{{ $program->program }}" @if(old('internCategory') == $program->program) selected @endif>{{ $program->program }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex flex-col">
                                ...
                                <input type="date" id="onboardingDate" name="onboardingDate" class="input-md input" value="{{ old('onboardingDate') }}"/>
                ...
                <div class="p-4 border-t flex justify-end">
                    <button form="addIntern" name="add" type="submit" class="btn btn-solid-success">Add Intern</button>
                </div>
            </div>
        </main>
